...
contract WaterfallPresale is Ownable {
    ...
    address public foilToken = 0x636F18AE1E46E81188A00beD283ED70c114e7502; //token to sell
    ...
    function claimTokens() external returns (uint256) {
        ...
        IERC20(foilToken).transfer(msg.sender, amountToSend);
    ...
    function recoverUnclaimed() external onlyOwner() {
        ...
        uint256 amountToSend = IERC20(foilToken).balanceOf(address(this));
        IERC20(foilToken).transfer(msg.sender, amountToSend);
    }
...
contract FOIL_Staking is Ownable {
    using SafeMath for uint256;

    address public foilToken = 0x636F18AE1E46E81188A00beD283ED70c114e7502; //token to stake
    ...
    function deposit(uint256 amountTokens) external {
        ...
        updateStake();
        IERC20(foilToken).transferFrom(msg.sender, address(this), amountTokens);
    ...
    function updateStake() public {
        ...
        if (totalStaked + dividends > maxTotalStaked) {
            IERC20(foilToken).transfer(msg.sender, dividends);
        } else {
    ...
    function unstake() external {
        ...
        IERC20(foilToken).transfer(msg.sender, toTransfer);
    ...
    function recoverFOIL() external onlyOwner() {
        require(
            block.timestamp >= (stakingEnd + 60 days),
            "recoverFOIL: too early"
        );
        uint256 amountToSend = IERC20(foilToken).balanceOf(address(this));
        IERC20(foilToken).transfer(msg.sender, amountToSend);
    }
